...
\begin{umlcall}[op={Click()}, return=ShowRoom(), padding=7]{User}{Client}
\begin{umlcall}[op={ActiveModuleSec()}]{Client}{ClientS} \end{umlcall}
\begin{umlcall}[op={CreateRoomSec()}]{Client}{Server} \end{umlcall}
\begin{umlcall}[op={showForm()}, return=ValidForm(), padding=7]{ClientS}{User} 
\end{umlcall}
\begin{umlcall}[op={SendForm()}]{ClientS}{ServerS} 
\begin{umlfragment}[type=alt, label=FormOK, inner xsep=5] 
\begin{umlcallself}[op={FillInForm()}]{ServerS} \end{umlcallself}
\begin{umlcall}[op={CheckCertificate()}, return=CheckCertificateOK(), padding=6]{ServerS}{LDD}\end{umlcall}
\begin{umlcallself}[op={GenerateKeys()}]{ServerS} \end{umlcallself}
\begin{umlcall}[op={BroadcastKeys()}]{ServerS}{ClientS1} \end{umlcall}
\begin{umlcall}[op={AlertInvit()}]{ClientS1}{User1} \end{umlcall}
\begin{umlcallself}[op={GenerateAdminKeys()}]{ServerS} \end{umlcallself} 
\begin{umlcallself}[op={CreateRoom()}]{ServerS} \end{umlcallself} 
\begin{umlcall}[op={ConnectRoom()}]{ServerS}{ClientS} \end{umlcall}
\begin{umlcall}[op={NotifyAdmin()}]{ServerS}{Admin} \end{umlcall}
\begin{umlcall}[op={ShowRoom()}]{ClientS}{Admin}\end{umlcall}
\end{umlfragment}
\end{umlcall}
\end{umlcall}
\end{umlseqdiag}
