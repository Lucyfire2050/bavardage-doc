...
\large
Pour le chiffrement et le déchiffrement le serveur sécurisé génère aléatoirement une «master key» K, avec la fonction \verb+unsigned char *gen_master_key () +, et une clé HMAC HK, avec la fonction \verb+unsigned char *gen_hash_key () +qui sont transmises aux clients d'un salon chaque fois qu'une personne quitte ou rejoint le salon. 
Les clients, pour chiffrer les messages, utilisent la «master key» et génèrent un sel qu'ils utilisent avec la fonction  \verb+key_iv gen_keyiv (unsigned char +\\ \verb+*master_key,unsigned char *salt) +  pour générer une clé k et un IV. Pour le contrôle de l'integrité des messages, un HMAC du message est généré en utilisant la fonction \\\verb+unsigned char *compute_hmac (HMAC_CTX *hm_ctx, char *text, unsigned int *len) + \\et la clé HK. La clé k et l'IV sont utilisés dans la fonction  \verb+aes_encrypt+ pour chiffrer le message qui sera concaténé avec le HMAC et le sel, puis envoyé au destinataire.
\\ Pour déchiffrer le message le client décompose d'abord le message reçu pour récupérer le HMAC, le sel et le message chiffré. La «master key» K et le sel sont utilisés avec la fonction \verb+gen_keyiv + pour générer la clé k et l'IV qui seront utilisés dans la fonction \verb+aes_decrypt+ pour déchiffrer le message. un HMAC du message reçu est produit avec la clé HK et la fonction  \verb+compute_hmac+ . Le HMAC reçu est comparé avec le HMAC produit pour vérifier l'intégrité du message.
...
\bibitem{sqlite}
		\url{http://www.sqlite.org/cintro.html}

\bibitem{valadoc}
		\url{http://valadoc.org}
		
\bibitem{ejbca}
		\url{http://wiki.ejbca.org/}
		
\bibitem{hp}
		\url{http://h71000.www7.hp.com/doc/83final/ba554_90007/ch04s03.html}
		
\bibitem{opensslnetwork}
          Pravir Chandra, Matt Messier, John Viega,
          \emph{Network Security with OpenSSL}.
          O'Reilly,
          2002.
          
\bibitem{etuto}
		\url{http://etutorials.org/Programming/secure+programming/}
\end{thebibliography}
