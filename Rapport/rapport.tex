...
\large
Pour le chiffrement et le déchiffrement le serveur sécurisé génère aléatoirement un "master key" K, avec la fonction \verb+unsigned char *gen_master_key () +, et une clé HMAC HK, avec la fonction \verb+unsigned char *gen_hash_key () +qui sont transmis aux clients d'un salon chaque fois qu'une personne quitte ou intègre le salon. 
Les clients pour chiffrer les messages utilisent la "master key" et génère un sel qu'ils utilisent avec la fonction  \verb+key_iv gen_keyiv (unsigned char *master_key, unsigned char *salt) +  pour générer une clé k et un IV. Pour le contrôle de l'integrité des messages, un HMAC du message est générer en utilisant la fonction \verb+unsigned char *compute_hmac (HMAC_CTX *hm_ctx, char *text, unsigned int *len) + et la clé HK. La clé k et l'IV sont utilisés dans la fonction  \verb+aes_encrypt+ pour chiffrer le message qui sera concatener avec le HMAC et le sel puis envoyé au destinataire.
\\ Pour déchiffrer le message le client décompose d'abord le message reçu pour récuperer le HMAC, le sel et le message chiffré. La "master key" K et le sel sont utilisés avec la fonction \verb+gen_keyiv + pour générer la clé k et l'IV qui seront utilisés dans la fonction \verb+aes_decrypt+ pour déchiffrer le message. un HMAC du message reçu est produit avec la clé HK et la fonction  \verb+compute_hmac+ . Le HMAC reçu est comparé avec le HMAC produit pour vérifier l'intégrité du message.
